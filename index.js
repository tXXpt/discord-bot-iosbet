    ...
    return interaction.editReply(`🗑️ Match deleted: ${removed.team1} vs ${removed.team2} (ID ${removed.id}) and bets refunded`);
          ...
          case 'bet': {
            ...
            if (![match.team1, match.team2, 'draw', 'Draw'].includes(team)) {
  return interaction.editReply('❌ Invalid team.');
}

// Normalize input
const normalizedTeam = team.toLowerCase() === 'draw' ? 'Draw' : team;
match.bets.push({ userId, team: normalizedTeam, amount });
